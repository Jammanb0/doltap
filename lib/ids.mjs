...
// 형식의 원본은 docs/document-graph-spec.md 의
// 「ID와 앵커」와 「관리 범위와 발급」다.
//
...
export function tryCreateLock(path, content, io = LOCK_IO, platform = process.platform) {
  let fd;
  for (let tries = 0; tries < 40; tries += 1) {
    try {
      ...
      // Windows 동시 발급 시험에서 잠금 open의 EPERM을 관찰했다.
      // 아직 소유하지 않은 파일에는 손대지 않고 배타적 생성만 짧게 다시 시도한다.
      if (platform === "win32" && error.code === "EPERM" && tries < 39) { pause(10); continue; }
      throw error;
    }
  }
