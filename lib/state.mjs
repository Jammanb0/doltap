...
function appendRecord(root, graph, group, cells) {
  ...
  // Review the planned state, including the first yearly review-file index.
  // The receipt itself is metadata and must not feed its own fingerprint.
  let effective = graph;
  if (changes.length && typeof cells === 'function') {
    const documents = new Map(collectDocuments(root).map(d => [d.path, d.text]));
    for (const c of changes) documents.set(c.path, c.after);
    documents.set(path, text);
    const planned = [...documents].map(([path, text]) => ({ path, text }));
  ...
  const row = '| ' + values.map(c => String(c ?? '').replace(/\|/g, '&#124;').replace(/[\r\n]+/g, ' ')).join(' | ') + ' |\n';
