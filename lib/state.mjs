...
export function readReviews(documents) {
  ...
  for (const d of [...documents].sort((a, b) => a.path.localeCompare(b.path))) {
    if (!/^\.doltap\/reviews\/\d{4}\.md$/.test(d.path)) continue;
    let group;
...
export function reviewStates(graph, records) {
  for (const n of graph.byId.values()) {
    const match = records.nodes.filter(r => r[1] === n.id && r[2] === n.contentHash.slice(0, 8) && r[3] === n.graphHash.slice(0, 8)).at(-1);
  ...
  for (const e of graph.edges) {
    ...
    const match = records.edges.filter(r => r[1] === e.id && (expires === 'to' || r[2] === a) && (expires === 'from' || r[3] === b)).at(-1);
...
export function stateFindings(graph, documents, root) {
  ...
  for (const [group, judgments] of [['nodes', ['최신임','고쳐야 함']], ['edges', ['반영함','영향 없음','재검증함']]]) {
    for (const r of records[group]) {
      if (r.length !== 7 || !/^[a-f0-9]{8}$/.test(r[2]) || !/^[a-f0-9]{8}$/.test(r[3]) || !judgments.includes(r[4]) || !r[5]?.trim() || !['사람','에이전트'].includes(r[6]) || (r[4] === '영향 없음' && [...r[5]].length < 10)) problems.push({ where: '.doltap/reviews', message: `잘못된 검토 기록: ${r[1]}` });
    }
  }
...
function appendRecord(root, graph, group, cells) {
  const path = `.doltap/reviews/${new Date().getUTCFullYear()}.md`;
  const rows = parseRegistry(readText(root, '.doltap/ids.md') ?? '');
  ...
  const row = '| ' + cells.map(c => String(c ?? '').replace(/\|/g, '&#124;').replace(/[\r\n]+/g, ' ')).join(' | ') + ' |\n';
...
export function reviewPlan(root, graph, id, { judgment, why, actor, node = false } = {}) {
  ...
  if (node) {
    ...
    if (!n || n.path.startsWith('.doltap/reviews/') || n.path === '.doltap/ids.md') throw new Error('검토할 본문 노드가 필요합니다');
    hashes = [n.contentHash.slice(0, 8), n.graphHash.slice(0, 8)];
  } else {
    const e = graph.edges.find(e => e.id === id);
    if (!e || !e.review.required) throw new Error('상태 검토가 필요한 관계가 아닙니다');
    hashes = edgeHashes(graph, e);
  }
  return appendRecord(root, graph, node ? 'nodes' : 'edges', [new Date().toISOString(), id, ...hashes, judgment, why.trim(), actor]);
}
