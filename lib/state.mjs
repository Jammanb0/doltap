...
export function stateFindings(graph, documents, root) {
  ...
  for (const row of rows) {
    ...
    if (row.state !== '삭제' && !node) problems.push({ where: '.doltap/ids.md', message: `발급한 범위가 사라졌습니다: ${row.id}. 이동은 doltap move-fix, 직접 삭제했다면 doltap delete-fix ${row.id} --why "삭제 이유"로 확인하세요` });
    if (!['활성','아카이브','삭제'].includes(row.state) || row.kind !== row.id.split('-')[1]) problems.push({where: '.doltap/ids.md', message:`잘못된 발급 상태·종류: ${row.id}`});
  }
  if(readText(root, '.doltap/ids.md') !== null) for(const n of graph.byId.values()) if(n.path!=='CLAUDE.md'&&!rows.some(r=>r.id===n.id))problems.push({where:n.path,message:`발급 기록에 없는 ID입니다: ${n.id}`});
  for (const e of graph.edges) {
