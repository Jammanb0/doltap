...
export function check(root) {
  ...
  if (agents === null) add('ENTRY_MISSING', PROBLEM, "AGENTS.md", "규칙 원본이 없습니다");
  if (claude === null) {
    add('CLAUDE_MISSING', PROBLEM, "CLAUDE.md", "없습니다. Claude Code가 규칙을 읽을 길이 없습니다");
  } else {
    ...
    if (!body.includes("@AGENTS.md")) {
      add('CLAUDE_IMPORT_MISSING', PROBLEM, "CLAUDE.md", "`@AGENTS.md` 가 없어 AGENTS.md로 이어지지 않습니다");
    } else if (body !== "@AGENTS.md") {
      add('CLAUDE_BODY', PROBLEM, "CLAUDE.md", "`@AGENTS.md` 한 줄이 아닙니다. 여기 적은 본문은 Codex가 읽지 못해 규칙이 갈라집니다");
    } else if (agents !== null) {
  ...
  if (!hasOperatingDir) {
    add('OPERATING_DIR_MISSING', PROBLEM, ".doltap/", "폴더가 없습니다");
  } else if (agents !== null) {
    if (meaningfulLines(agents).some((l) => l.text.includes(".doltap/"))) {
      passed.push("AGENTS.md → .doltap/ 안내");
    } else {
      add('OPERATING_NAVIGATION_MISSING', PROBLEM, "AGENTS.md", "`.doltap/` 로 이어지는 안내가 없어 그 폴더를 아무도 읽지 않습니다");
    }
  }
  ...
  if (hasOperatingDir) {
    for (const path of REQUIRED) {
      ...
      missing += 1;
      add('REQUIRED_DOCUMENT_MISSING', PROBLEM, path, "없습니다. 골격의 필수 문서입니다");
    }
  ...
  for (const doc of docs) {
    for (const { number, text } of meaningfulLines(readFileSync(doc, "utf8"))) {
      for (const token of documentTokens(text)) {
        ...
        broken += 1;
        add('DOCUMENT_PATH_MISSING', PROBLEM, at(doc, number), `가리키는 ${token} 가 없습니다`);
      }
    }
  }
  ...
  for (const doc of docs) {
    ...
    blanks += hits.length;
    add('PLACEHOLDER', NOTICE, at(doc), `아직 채우지 않은 자리 ${hits.length}곳 (${hits.slice(0, 5).join(", ")}${hits.length > 5 ? " …" : ""}행)`);
  }
  ...
  for (const name of active) {
    if (!/^\d{3}-[a-z0-9-]+$/.test(name)) {
      ...
      add('WORKSTREAM_NAME', NOTICE, at(join(plansDir, name)), "이름이 `<세 자리 번호>-<영문 소문자>` 형식이 아닙니다");
    }
    const number = name.slice(0, 3);
    const clash = numbers.get(number);
    if (clash) {
      workstreamClean = false;
      add('WORKSTREAM_NUMBER_DUPLICATE', PROBLEM, at(join(plansDir, name)), `번호 ${number} 가 ${clash} 와 겹칩니다`);
    } else {
  ...
  for (const name of active) {
    for (const required of WORKSTREAM_REQUIRED) {
      ...
      workstreamClean = false;
      add('WORKSTREAM_DOCUMENT_MISSING', PROBLEM, at(join(plansDir, name)), `${required} 가 없습니다`);
    }
  }
  ...
  if (current !== null) {
    for (const name of active) {
      ...
      workstreamClean = false;
      add('WORKSTREAM_UNLISTED', PROBLEM, ".doltap/plans/current.md", `${name} 이 없어 새 세션이 찾지 못합니다`);
    }
    ...
    for (const name of listedWorkstreams(current)) {
      ...
      add('WORKSTREAM_TARGET_MISSING', PROBLEM, ".doltap/plans/current.md", archived ? `${name} 은 이미 아카이브에 있습니다. current.md의 활성 색인을 갱신하세요` : `${name} 이 적혀 있는데 그 폴더가 없습니다`);
    }
  }
  ...
  for (const doc of docs) {
    ...
    const hits = readFileSync(doc, "utf8")
      ...
      .filter(Boolean);
    if (hits.length) add('TEMP_REFERENCE', NOTICE, at(doc), `\`.doltap-bootstrap\` 참조가 ${hits.length}곳 남아 있습니다 (${hits.join(", ")}행)`);
  }
  const setup = active.find((name) => name.endsWith(SETUP_SUFFIX));
  if (setup) {
    add('SETUP_IN_PROGRESS', NOTICE,
...
export function format(result) {
  const lines = [];
  // 긴 경로가 다른 항목의 설명까지 오른쪽으로 밀지 않도록 위치를 별도 줄에 둔다.
  const indent = '    ';
  const entry = (f) => {
    const prefix = f.code ? `[${f.code}] ` : '';
    return [
      `  ${prefix}${f.where}`,
      ...f.message.split('\n').map((line) => `${indent}${line}`),
      ...(f.related ?? []).map((r) => `${indent}관련: ${r.path}${r.line ? ':' + r.line : ''} — ${r.label}`),
      ...(f.hint ? [`${indent}다음: ${f.hint}`] : []),
    ];
  };
