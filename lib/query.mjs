...
export function audit(root, result, records, scope, { budget = 8000, changed = false, includeLegacy = false, relation, state } = {}) {
  const prefix = scope === '.' ? '' : scope.replace(/\\/g, '/').replace(/\/$/, '');
  ...
  for (const n of result.graph.byId.values()) {
    if (!inside(n.path) || (!includeLegacy && n.state === 'legacy') || (state && n.state !== state) || (changed && n.review.state === 'fresh' && n.review.judgment !== '고쳐야 함')) continue;
  ...
  if (prefix && !prefix.startsWith('.doltap')) visit(prefix);
  return { schema: 'doltap.audit.v1', scope, budget: { unit: 'chars', scope: 'body', limit: budget, used }, errors: result.problems.filter(p => inside(p.where.split(':')[0])), reviewRequired: result.graph.edges.filter(e => e.review?.state === 'stale' && (inside(e.path) || inside(result.graph.byId.get(e.to)?.path ?? ''))), candidates, suggestions: [], ambiguous: [], decisions: records.suggestions, omitted };
}
