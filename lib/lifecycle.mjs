...
import { discoverDocuments, resolveTarget } from './graph-check.mjs';
...
function decisionProblems(node) {
  ...
  const close = () => {
    ...
    if (message) problems.push(diagnostic('ARCHIVE_DECISION', `${node.path}:${open.line}`, `${message}: ${open.title}`));
    open = null;
  };
...
export function archiveCheck(graph, scope) {
  ...
  for (const n of graph.byId.values()) {
    ...
    if (message) {
      const code = states.length !== 1 ? 'ARCHIVE_STATE' : state === '미해결' ? 'ARCHIVE_UNRESOLVED'
        : state === '이월' ? 'ARCHIVE_HANDOFF' : 'ARCHIVE_REASON';
      problems.push(diagnostic(code, `${n.path}:${n.startLine}`, `${message}: ${n.id}`));
    }
  }
  return problems;
}
export function deletePlan(root, graph, id, { mode, replacement, why } = {}) {
  ...
  // 일반 링크는 관계처럼 대체할 수 없으므로 본문에서 직접 정리하게 한다.
  for (const doc of graph.documents) for (const link of doc.links) {
    if (isExternal(link.dest) || (doc.path === n.path && link.line >= n.startLine && link.line <= n.endLine)) continue;
    const target = splitTarget(link.dest);
    const pointsToFile = n.kind === 'd' && target.path && resolveTarget(root, doc.path, target.path) === n.path;
    if (ids.has(idFromFragment(target.fragment)) || pointsToFile) throw new Error(`일반 링크는 직접 정리하세요: ${doc.path}:${link.line} ${link.dest}`);
  }
