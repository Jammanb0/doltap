...
import { parseDocument, RELATIONS, markVerbatim, splitTarget, isExternal } from './graph.mjs';
...
export function migratePlan(root, { legacy = readText(root, '.doltap/ids.md') === null } = {}) {
  ...
  const oldProject = '.doltap/project.md', newProject = '.doltap/plans/project.md';
  if (source.some(d => d.path === oldProject)) {
    if (source.some(d => d.path === newProject)) throw new Error('project.md가 두 위치에 있습니다. 합칠 내용을 먼저 정하세요');
    const knownPaths = new Set(source.map(d => d.path));
    source = source.map(doc => {
      const path = doc.path === oldProject ? newProject : doc.path;
      let text = doc.text;
      if (!doc.path.startsWith('.doltap/reviews/') && !doc.path.includes('/archive/legacy/') && !(legacy && doc.path.startsWith('.doltap/archive/workstreams/'))) {
        const parsed = parseDocument(text, doc.path);
        for (const link of [...parsed.edges, ...parsed.links]) {
          if (isExternal(link.dest)) continue;
          const target = splitTarget(link.dest);
          let resolved = target.path ? posix.normalize(posix.join(posix.dirname(doc.path), target.path)) : doc.path;
          if (!knownPaths.has(resolved) && knownPaths.has(target.path)) resolved = target.path;
          if (resolved !== oldProject && path === doc.path) continue;
          if (resolved === oldProject) resolved = newProject;
          const dest = (posix.relative(posix.dirname(path), resolved) || posix.basename(resolved)).split('/').map(encodeURIComponent).join('/') + (target.fragment ? '#' + target.fragment : '');
          text = text.split(`](${link.dest})`).join(`](${dest})`);
        }
        text = text.replaceAll(oldProject, newProject);
      }
      return { ...doc, path, originalPath: doc.originalPath ?? doc.path, text };
    });
  }
  ...
  for (const [path, file] of files) {
    ...
    if (!path.includes('/archive/legacy/') && !path.startsWith('.doltap/reviews/')) for (const [old, fresh] of moved) file.text = file.text.split(old).join(fresh);
