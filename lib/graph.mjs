...
// 형식의 원본은 docs/document-graph-spec.md 다.
//
...
function readAnchor(line) {
  ...
  // 부분 문자열로 속성을 찾으면 data-name/data-id 를 실제 name/id 로 오인한다.
  // 속성 전체를 앞에서부터 읽고, 표준 형식에 없는 속성과 중복도 거부한다.
  const seen = new Map();
  let rest = shell[1];
  while (rest.trim()) {
    const attribute = /^\s*([^\s=]+)\s*=\s*"([^"]*)"/.exec(rest);
    if (!attribute) return { malformed: `속성 문법을 읽을 수 없습니다: ${rest.trim()}` };
    const [, key, value] = attribute;
    if (key !== "name" && key !== "id") return { malformed: `허용되지 않은 속성입니다: ${key}` };
    if (seen.has(key)) return { malformed: `${key} 속성이 중복됐습니다` };
    seen.set(key, value);
...
export function markVerbatim(lines) {
...
export function splitTarget(dest) {
  ...
  let path = rawPath;
  try { path = decodeURIComponent(rawPath); } catch { /* A literal percent may be a file name. */ }
  return { path, fragment: at === -1 ? null : dest.slice(at + 1) };
}
...
export function parseDocument(text, path) {
  ...
  lines.forEach((line, i) => {
    ...
    if (anchor.malformed) {
      problems.push({ ...at, code: 'ANCHOR_MALFORMED', message: `앵커 형식이 아닙니다 — ${anchor.malformed}` });
      return;
    }
    ...
    if (!top) {
      problems.push({ ...at, code: 'ANCHOR_START_MISSING', message: `${anchor.id} 의 끝 앵커만 있고 시작이 없습니다` });
      return;
    }
    ...
    if (top.id !== anchor.id) {
      problems.push({
        ...at,
        code: 'ANCHOR_CROSSED',
        related: [{ path, line: top.startLine, label: '아직 닫히지 않은 시작' }],
  ...
  for (const left of open) {
    problems.push({ path, line: left.startLine, code: 'ANCHOR_END_MISSING', message: `${left.id} 의 범위가 닫히지 않았습니다` });
  }
