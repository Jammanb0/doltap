...
function readAnchor(line) {
  ...
  const values = [];
  for (const [, , value] of shell[1].matchAll(ATTR)) values.push(value);
  if (!values.length) return { malformed: "name 도 id 도 없습니다" };
  // name 과 id 를 함께 쓰되 값이 같아야 한다. 다르면 어느 쪽이 정체인지 모른다.
...
export function buildGraph(documents) {
  ...
  for (const doc of documents) {
    for (const raw of doc.edges) {
      const to = idFromFragment(splitTarget(raw.dest).fragment);
