...
//
// 규칙의 원본은 docs/document-graph-spec.md 다.

import { existsSync, lstatSync, readFileSync, readdirSync } from "node:fs";
import { dirname, isAbsolute, join, relative, resolve, sep } from "node:path";

import { RELATIONS, buildGraph, idFromFragment, isExternal, parseDocument, splitTarget } from "./graph.mjs";
import { parseRegistry } from './ids.mjs';
import { diagnostic, diagnosticJson, related } from './diagnostics.mjs';
...
export function collectDocuments(root) {
  ...
  for (const doc of base.map((d) => parseDocument(d.text, d.path))) {
    for (const edge of doc.edges) {
      ...
      if (!resolved) {
        // Only recover an explicitly referenced identity, not arbitrary documents.
        candidates ??= (()=>{const out=[];walk(root,root,out);return out;})();
        const wanted=idFromFragment(fragment);
        for(const candidate of candidates) {
          if(known.has(candidate)||!isManagedMarkdown(root,candidate))continue;
          const found=read(root,candidate);
          if(parseDocument(found.text,found.path).ranges.some(r=>r.id===wanted)){known.add(candidate);extra.push(found);}
        }
        continue;
      }
      if (known.has(resolved) || !isManagedMarkdown(root, resolved)) continue;
      known.add(resolved);
...
// 사후 삭제 확인은 색인에서 빠진 이동 문서도 찾는다. 복구 사본·숨김 경로는 제외한다.
export function discoverDocuments(root) {
  const paths = [];
  walk(root, root, paths);
  const operating = join(root, OPERATING_DIR);
  if (existsSync(operating)) walk(root, operating, paths);
  return [...new Set(paths)].filter(path => isManagedMarkdown(root, path)).map(path => read(root, path));
}
...
const OUTSIDE = "밖";
export function resolveTarget(root, fromPath, target) {
...
export function checkGraph({ root, documents, entryPoints = [] }) {
  ...
  if (root && entryPoints.includes("CLAUDE.md")) {
    ...
    if (!existsSync(claudePath) || !lstatSync(claudePath).isFile()) {
      preflight.push({ kind: PROBLEM, ...diagnostic('CLAUDE_MISSING', 'CLAUDE.md', "없습니다. Claude Code가 규칙을 읽을 길이 없습니다") });
    } else {
      ...
      if (!body.includes("@AGENTS.md")) {
        preflight.push({ kind: PROBLEM, ...diagnostic('CLAUDE_IMPORT_MISSING', 'CLAUDE.md', "`@AGENTS.md` 가 없어 AGENTS.md로 이어지지 않습니다") });
      } else if (body !== "@AGENTS.md") {
        preflight.push({ kind: PROBLEM, ...diagnostic('CLAUDE_BODY', 'CLAUDE.md', "`@AGENTS.md` 한 줄이 아닙니다. 여기 적은 본문은 Codex가 읽지 못해 규칙이 갈라집니다") });
      } else {
        claudeValid = true;
      }
    }
  ...
  for (const row of parseRegistry(documents.find(d => d.path === '.doltap/ids.md')?.text ?? '')) {
    const live = graph.byId.get(row.id);
    if (row.state === '삭제' && live) {
      const text = documents.find(d=>d.path===live.path)?.text ?? '';
      const body = text.split(/\r?\n/).slice(live.startLine,live.endLine-1).join('\n').trim();
      if (body.startsWith('`삭제 표식` ')) Object.assign(live, { deleted: true, lastPath: row.path, replacedBy: row.replacedBy });
    }
    if (row.state === '삭제' && !graph.byId.has(row.id)) graph.byId.set(row.id, {
      id: row.id, kind: row.kind, path: '.doltap/ids.md', startLine: 1, endLine: 1,
      parent: null, depth: 0, deleted: true, lastPath: row.path, replacedBy: row.replacedBy,
    });
  }
  const findings = [...preflight];
  const add = (code, kind, where, message, extra) => findings.push({ kind, ...diagnostic(code, where, message, extra) });
  const at = (path, line) => (line ? `${path}:${line}` : path);

  // 1. 파서가 문서 안에서 이미 잡은 것 — 앵커 형식, 닫히지 않은 범위, 교차
  for (const doc of parsed) {
    for (const p of doc.problems) add(p.code, PROBLEM, at(p.path, p.line), p.message, { related: p.related ?? [] });
  }

  // 2. ID 는 관리 범위 전체에서 하나만 가리켜야 한다.
  // 겹치면 링크 목적지, 이동 복구, 삭제 대상이 모두 모호해진다.
  for (const dup of graph.duplicates) {
    add('ID_DUPLICATE', PROBLEM,
      at(dup.again.path, dup.again.startLine),
      `ID ${dup.id} 가 ${at(dup.first.path, dup.first.startLine)} 에도 있습니다`,
      { related: [related(dup.first, '먼저 발견한 범위')] }
    );
  }
  ...
  for (const doc of parsed) {
    ...
    if (roots.length === 1 && doc.path !== 'CLAUDE.md') {
      const original = documents.find(d => d.path === doc.path)?.text ?? '';
      const lines = original.split(/\r?\n/);
      const before = lines.slice(0, roots[0].startLine - 1).join('\n').replace(/^---\n[\s\S]*?\n---\n?/, '').replace(/<!--[\s\S]*?-->/g, '').trim();
      const after = lines.slice(roots[0].endLine).join('\n').replace(/<!--[\s\S]*?-->/g, '').trim();
      if (before || after) add('DOCUMENT_BODY_OUTSIDE', PROBLEM, doc.path, '문서 ID 범위 밖에 본문이 있습니다');
    }
    for (const range of doc.ranges) {
      if (range.kind === "d" && range.depth !== 0) {
        add('DOCUMENT_NESTED', PROBLEM, at(range.path, range.startLine), `${range.id} 는 문서 노드인데 다른 범위 안에 있습니다`);
      }
      if (range.kind !== "d" && range.depth === 0) {
        add('RANGE_OUTSIDE_DOCUMENT', PROBLEM,
    ...
    if (roots.length > 1) {
      add('DOCUMENT_MULTIPLE', PROBLEM, at(doc.path, roots[1].startLine), "문서 노드가 둘 이상입니다");
    }
  }
  ...
  for (const edge of graph.edges) {
    ...
    if (!meta) {
      add('RELATION_TYPE', PROBLEM, where, `허용되지 않은 관계 유형입니다: \`${edge.type}\``);
      continue;
    }
    if (!edge.from) {
      add('RELATION_SOURCE_MISSING', PROBLEM, where, `관계가 어느 범위에도 들어 있지 않습니다: \`${edge.type}\``);
    }
    // 남의 주소의 조각은 이 프로젝트의 노드를 가리킬 수 없다.
    if (isExternal(edge.dest)) {
      add('RELATION_EXTERNAL', PROBLEM, where, `관계 대상이 외부 주소입니다: ${edge.dest}`);
      continue;
    }
    const { path, fragment } = splitTarget(edge.dest);
    const resolved = resolveTarget(root, edge.path, path);
    if (resolved === false || resolved === OUTSIDE) {
      add(resolved === OUTSIDE ? 'TARGET_OUTSIDE_ROOT' : 'RELATION_FILE_MISSING', PROBLEM, where, missingMessage("관계", resolved, path));
    }
    if (!edge.to) {
      // 관계에는 ID 조각만 받는다. 제목 조각은 제목이 바뀌면 깨진다.
      add('RELATION_FRAGMENT', PROBLEM, where, `관계 대상이 doltap ID 조각이 아닙니다: #${fragment ?? ""}`);
      continue;
    }
    if (resolved && resolved !== OUTSIDE && !isManagedMarkdown(root, resolved)) {
      add('RELATION_UNMANAGED', PROBLEM, where, `관계 대상이 그래프 관리 범위 밖입니다: ${resolved}`);
      continue;
    }
    if (!graph.byId.has(edge.to)) {
      add('RELATION_ID_MISSING', PROBLEM, where, `관계가 가리키는 ID 가 없습니다: ${edge.to}`);
      continue;
    }
    if (graph.byId.get(edge.to).deleted && edge.type !== 'supersedes') {
      add('RELATION_DELETED', PROBLEM, where, `삭제된 ID를 가리킵니다: ${edge.to}`);
    }
    ...
    if (resolved === false || (resolved && resolved !== OUTSIDE && resolved !== graph.byId.get(edge.to).path)) {
      add('RELATION_PATH_STALE', NOTICE,
        where,
        `${edge.to} 는 ${graph.byId.get(edge.to).path} 에 있습니다 — 경로가 낡았을 수 있습니다`,
        { related: [related(graph.byId.get(edge.to), '대상 ID의 현재 위치')] }
      );
    }
    ...
    if (edge.from && edge.id) {
      if (declared.has(edge.id)) {
        add('RELATION_DUPLICATE', PROBLEM,
          where,
          meta.symmetric
  ...
  for (const doc of parsed) {
    for (const link of doc.links) {
      ...
      if (!path) {
        ...
        if (id && (!graph.byId.has(id) || graph.byId.get(id).path !== doc.path)) add('LINK_LOCAL_ID_MISSING', PROBLEM, at(link.path, link.line), `같은 문서에 링크 ID가 없습니다: ${id}`);
        continue;
      }
      const target = resolveTarget(root, link.path, path);
      if (target === false || target === OUTSIDE) {
        add(target === OUTSIDE ? 'TARGET_OUTSIDE_ROOT' : 'LINK_FILE_MISSING', PROBLEM, at(link.path, link.line), missingMessage("링크", target, path));
        continue;
      }
      const id = idFromFragment(fragment);
      if (id && !graph.byId.has(id)) {
        add('LINK_ID_MISSING', PROBLEM, at(link.path, link.line), `링크가 가리키는 ID 가 없습니다: ${id}`);
      }
      else if (id && graph.byId.get(id).path !== target) {
        add('LINK_PATH_MISMATCH', PROBLEM, at(link.path, link.line), `링크의 경로에 ID가 없습니다: ${id} — ${graph.byId.get(id).path} 에 있습니다`,
          { related: [related(graph.byId.get(id), '대상 ID의 현재 위치')] });
      }
    }
  }
  ...
  if (entryPoints.length) {
    ...
    for (const doc of parsed) {
      const docRoot = doc.ranges.find((r) => r.depth === 0);
      if (!docRoot) {
        add('DOCUMENT_ID_MISSING', PROBLEM, doc.path, "문서 노드가 없습니다 — d 범위로 감싸세요");
        continue;
      }
      if (!docRoot.deleted && !reachable.has(docRoot.id)) {
        add('DOCUMENT_UNREACHABLE', PROBLEM, doc.path, `진입점에서 닿지 않습니다 (${docRoot.id})`);
      }
    }
    for (const edge of graph.edges) {
      if (edge.path === '.doltap/plans/current.md' && graph.byId.get(edge.to)?.path.includes('/archive/')) add('CURRENT_POINTS_ARCHIVE', PROBLEM, at(edge.path,edge.line), '현재 색인이 아카이브를 활성 대작업으로 가리킵니다');
    }
    for (const doc of parsed.filter(d => /\.doltap\/(?:plans\/workstreams|archive\/(?:legacy\/)?workstreams)\/[^/]+\/README\.md$/.test(d.path))) {
      const archived = doc.path.includes('/archive/');
      const index = parsed.find(d=>d.path === `.doltap/plans/${archived ? 'history' : 'current'}.md`);
      if (!index) continue;
      const target = doc.ranges.find(r=>r.kind==='d');
      const seen = new Set(), queue = index.ranges.filter(r=>r.kind==='d').map(r=>r.id);
      while(queue.length) {
        const id=queue.shift(); if(seen.has(id)) continue; seen.add(id);
        for(const e of graph.outgoing.get(id)??[]) queue.push(e.to);
        for(const n of graph.byId.values()) if(n.parent===id) queue.push(n.id);
      }
      if(target && !seen.has(target.id)) add('WORKSTREAM_INDEX_UNREACHABLE', PROBLEM,doc.path,`${archived ? 'history.md' : 'current.md'}에서 대작업에 닿지 않습니다`);
    }
  }
...
export function toJson(result) {
  return {
    ...
    problems: result.problems.map(diagnosticJson),
    notices: result.notices.map(diagnosticJson),
