...
export function checkGraph({ root, documents, entryPoints = [] }) {
  ...
  for (const doc of parsed) {
    ...
    if (roots.length === 1 && doc.path !== 'CLAUDE.md') {
      ...
      if (before || after) add(PROBLEM, doc.path, '문서 ID 범위 밖에 본문이 있습니다');
    }
  ...
  for (const edge of graph.edges) {
    ...
    if (graph.byId.get(edge.to).deleted && edge.type !== 'supersedes') {
      add(PROBLEM, where, `삭제된 ID를 가리킵니다: ${edge.to}`);
    }
  ...
  for (const doc of parsed) {
    for (const link of doc.links) {
      ...
      if (!path) {
        ...
        if (id && (!graph.byId.has(id) || graph.byId.get(id).path !== doc.path)) add(PROBLEM, at(link.path, link.line), `같은 문서에 링크 ID가 없습니다: ${id}`);
        continue;
      }
      ...
      if (id && !graph.byId.has(id)) {
        ...
        add(PROBLEM, at(link.path, link.line), `링크의 경로에 ID가 없습니다: ${id} — ${graph.byId.get(id).path} 에 있습니다`);
      }
    }
  }
  ...
  if (entryPoints.length) {
    ...
    for (const edge of graph.edges) {
      if (edge.path === '.doltap/plans/current.md' && graph.byId.get(edge.to)?.path.includes('/archive/')) add(PROBLEM, at(edge.path,edge.line), '현재 색인이 아카이브를 활성 대작업으로 가리킵니다');
    }
    for (const doc of parsed.filter(d => /\.doltap\/(?:plans\/workstreams|archive\/(?:legacy\/)?workstreams)\/[^/]+\/README\.md$/.test(d.path))) {
      ...
      if(target && !seen.has(target.id)) add(PROBLEM,doc.path,`${archived ? 'history.md' : 'current.md'}에서 대작업에 닿지 않습니다`);
    }
  }
