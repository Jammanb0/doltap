...
test('일반 ID 링크도 잘못된 파일을 가리키면 오류다',()=>{
 const root=project(),id=documentId(inspect(root),'.doltap/plans/project.md');
...
test('이관은 미리보기에서 쓰지 않으며 재실행해도 ID가 같다', () => {
  const root = fixture({'AGENTS.md':'# rules','.doltap/plans/project.md':'# project','CLAUDE.md':'@AGENTS.md\n'});
...
test('검토는 현재 내용에 묶이며 관계 추가는 이웃 검토를 만료시키지 않는다',()=>{
  const root=project(); let r=inspect(root); const a=documentId(r,'AGENTS.md'), b=documentId(r,'.doltap/plans/project.md'), c=documentId(r,'.doltap/plans/ideas.md');
  ...
  const p=join(root,'.doltap/plans/project.md'); writeFileSync(p,readFileSync(p,'utf8').replace('# 프로젝트','# 바뀐 프로젝트'));
...
test('삭제는 문서 안의 자식으로 들어오는 관계도 거부한다',()=>{
  const root=project();mutate(root,()=>idPlan(root,'.doltap/plans/project.md',{kind:'s',at:'한 줄 요약'}));let r=inspect(root);const child=[...r.graph.byId.values()].find(n=>n.kind==='s');
  mutate(root,()=>linkPlan(root,documentId(r,'AGENTS.md'),child.id,'references'));r=inspect(root);
  assert.throws(()=>deletePlan(root,r.graph,documentId(r,'.doltap/plans/project.md'),{mode:'purge',why:'삭제'}),/참조 중/);
});
...
test('project 설명 이관은 ID와 안팎의 상대 링크를 보존하고 반복 실행할 수 있다',()=>{
 ...
 assert.ok(fullCheck(root).problems.some(p=>p.message.includes('doltap migrate')));
 const plan=migratePlan(root);assert.ok(readText(root,old));assert.equal(readText(root,fresh),null);
 transact(root,plan);const result=fullCheck(root);
 assert.deepEqual(result.problems,[]);assert.equal(documentId(result,fresh),id);assert.equal(readText(root,old),null);
 assert.ok(readText(root,fresh).includes(`](ideas.md#${idea}-start)`));assert.ok(readText(root,fresh).includes('](../rules/verification.md)'));
 assert.ok(readText(root,'.doltap/plans/ideas.md').includes(`](project.md#${id}-start)`));
 assert.ok(readText(root,'.doltap/ids.md').includes(fresh));
 assert.equal(readText(root,recordPath),record);
 assert.equal(migratePlan(root).filter(c=>c.before!==c.after).length,0);
});
test('project 설명이 두 위치에 있으면 자동 이관이 원본을 덮어쓰지 않는다',()=>{
 const root=project(), fresh=readText(root,'.doltap/plans/project.md');
 writeFileSync(join(root,'.doltap/project.md'),'# 별도로 작성한 설명');
 assert.throws(()=>migratePlan(root),/두 위치/);
 assert.equal(readText(root,'.doltap/plans/project.md'),fresh);assert.equal(readText(root,'.doltap/project.md'),'# 별도로 작성한 설명');
});
test('구형 운영 폴더는 안내하고 복구 가능한 이관을 제공한다',()=>{
 ...
 assert.ok(readText(root,'.doltap/plans/project.md'));assert.equal(readText(root,'.agents/project.md'),null);
...
test('이동 복구는 라벨과 ID를 유지하고 경로와 발급 위치를 갱신한다',()=>{
 const root=project();let r=inspect(root);const id=documentId(r,'.doltap/plans/project.md');
 const text=readText(root,'.doltap/plans/project.md');transact(root,[change(root,'.doltap/plans/project.md',null),change(root,'.doltap/renamed.md',text)]);
...
test('중복 ID이면 이관과 경로 복구 모두 거부한다',()=>{
 const root=project();writeFileSync(join(root,'.doltap/copy.md'),readText(root,'.doltap/plans/project.md'));
...
test('삭제 표식은 내용 대신 ID와 이유를 남기고 이후 발급에서도 보존된다',()=>{
 ...
 mutate(root,()=>idPlan(root,'.doltap/plans/project.md'));assert.ok(readText(root,'.doltap/ids.md').includes('더 이상 사용하지 않는 문서'));
 assert.throws(()=>idPlan(root,'.doltap/unused.md'),/삭제/);
});
test('대체 삭제는 참조를 옮기고 이전 ID로 supersedes를 보존한다',()=>{
 const root=project();let r=inspect(root);const old=documentId(r,'.doltap/plans/ideas.md'),fresh=documentId(r,'.doltap/plans/project.md');
...
test('대칭 검토는 선언 방향이 바뀌어도 같은 영수증을 사용한다',()=>{
 const root=project();let r=inspect(root);const a=documentId(r,'.doltap/plans/project.md'),b=documentId(r,'.doltap/plans/ideas.md');mutate(root,()=>linkPlan(root,a,b,'update-with'));
 ...
 let text=readText(root,'.doltap/plans/project.md').replace(/^- `update-with`.*\n/gm,'');writeFileSync(join(root,'.doltap/plans/project.md'),text);mutate(root,()=>linkPlan(root,b,a,'update-with'));
...
test('감사 판단 기록은 누적되며 --changed는 노드 검토를 사용한다',()=>{
 const root=project();let r=inspect(root);const a=documentId(r,'.doltap/plans/project.md'),b=documentId(r,'.doltap/plans/ideas.md');
 ...
 assert.equal(r.records.suggestions.length,1);assert.equal(audit(root,r,r.records,'.doltap/plans/project.md',{changed:true}).candidates.length,0);
});
test('문서 밖 본문과 주석 속 가짜 앵커를 구분한다',()=>{
 const root=project();const path=join(root,'.doltap/plans/project.md');writeFileSync(path,readText(root,'.doltap/plans/project.md')+'outside');assert.ok(inspect(root).problems.some(p=>p.message.includes('범위 밖')));
});
