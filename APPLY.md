...
및 `.doltap/archive/legacy/workstreams/`에 이미 쓰인 번호를 모두 확인해서 정합니다. 모두 없으면 `001`입니다. 세팅
...
2. 세팅 워크스트림 폴더를 만듭니다. 이 워크스트림과 2단계에서 기록 이관으로
   ...
   두지 않아도 됩니다. 결정마다 `계속 유효`인지 `이번만`인지 한 줄 붙입니다 —
   문법은 `.doltap/plans/workstreams.md`의 「진행하는 동안」에 있습니다.
...
기존 `.agents/`를 만났으면 이 절을 먼저 읽습니다. `.doltap/`로의 경로 이관과
문서 ID·색인 추가는 `node <골격 위치>/bin/doltap.mjs migrate .`로 미리봅니다.
사용자 승인 범위에서 `--apply`를 실행합니다. 두 운영 폴더가 함께 있거나 마크다운
외 파일이 있으면 자동으로 합치지 않으므로, 이 문서의 조사·충돌 확인부터 합니다.
과거 산문과 명령은 보존하고 현재 링크·색인만 고칩니다. 임의의 0.1.x 문서 역할을
의미에 맞게 자동 분류하지는 않습니다.

기존 `.doltap/project.md`도 같은 migrate 명령으로 `.doltap/plans/project.md`로
옮깁니다. 기존 ID와 문서 링크를 유지하고 두 위치에 파일이 있으면 덮어쓰지 않습니다.
미리보기에서 본문 안의 경로 설명도 확인합니다.

이관은 제목별 section을 자동 생성하지 않습니다. 문서 ID와 탐색 색인부터 만들고
필요한 주장에만 `id`·`link`를 사용합니다. 기존 파일을 부분 복사할 때 템플릿의
발급 기록을 실제 문서 ID와 다른 상태로 덮어쓰지 않습니다.
