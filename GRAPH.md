...
워크스트림 `decisions.md`의 결정(`##` 절)마다 `계속 유효` 또는 `이번만`을 하나
선언합니다. `계속 유효`에는 담당 활성 원본을 같은 줄에 적고, 아직 옮기지 않았으면
옮길 곳을 적습니다. 검사는 표식과 원본 표기의 유무만 봅니다. 적어 둔 원본이 실제로
그 내용을 담았는지는 확인하지 않으므로 옮겼는지는 사람이 대조합니다.
