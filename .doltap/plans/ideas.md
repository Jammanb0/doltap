...
- **살아 있는 범위의 묶음 삭제.** `delete`는 ID 하나씩이고 `delete-fix`는 이미
  사라진 것의 사후 정리입니다. 아카이브 워크스트림 하나를 정식으로 지우려면 참조
  제거, ID 수만큼 `delete --apply`, 빈 폴더 정리, `check`를 손으로 이어야 합니다.
  번거로워서 사람이 `rm`을 쓰게 되고 그래서 사후 정리가 필요해집니다. 폴더를 받아
  영향과 순서를 한 번에 보여주고 적용하는 예방 쪽을 검토합니다.
