...
`doltap init`과 `doltap check`는 필요할 때 부르는 명령입니다. 운영 문서의 원본은
마크다운이므로 도구가 사라져도 사람이 직접 읽고 관리할 수 있습니다.
CLI로 문서를 편집하면 별도로 `.doltap/recovery/`에 `.bak`·JSON 복구 자료가
생길 수 있으며, 이것이 전용 런타임을 요구하는 것은 아닙니다.
