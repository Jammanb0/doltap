...
모든 삭제 모드에서 범위 밖의 일반 Markdown 링크는 직접 정리해야 합니다.
삭제할 ID·자식 ID로 향하는 링크와, 문서 전체를 삭제할 때 그 파일로 향하는 링크가
해당합니다. 같은 문서 안이어도 삭제 범위 밖에서 들어오는 링크는 확인합니다.
검사 대상은 관리 문서에 있는 지원 링크 구문이며, 미등록 문서 전체를 검색하지는 않습니다.
