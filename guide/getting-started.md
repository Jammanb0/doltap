...
운영 문서를 부모에 두거나 회사 레포의 지시 파일을 보존해야 한다면
[구성 사례](adoption-layouts.md)를 참고합니다. APPLY에서 위치·변경 범위·보관 방식과
에이전트 환경을 먼저 조사하고 이번에 적용할 단계와 해당 없는 단계를 정합니다.

```text
doltap의 APPLY.md를 읽고 운영 문서를 둘 위치와 기존 규칙·기록을 조사해줘.
보존해야 할 파일과 기록의 공유 범위를 확인하고 변경안을 먼저 보여줘.
```
