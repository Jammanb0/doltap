...
[사용 안내](README.md) · [개념](concepts.md) · [대작업 흐름](workstreams.md)

상황별 안내 뒤에 세부 동작을 찾아볼 때 읽습니다. 훅 설정은 [선택적 훅](hooks.md)에 있습니다.
...
위 ID·관계 편집은 미리보기가 기본이고, 같은 명령에 `--apply`를 붙여 적용합니다.
...
전에는 다른 쓰기를 막습니다. 일반 편집의 미리보기는 다음 실행에 저장되지 않습니다.
`--apply`는 그때의 파일로 변경안을 다시 계산하므로 미리보기 뒤 파일을 고쳤다면
다시 미리보세요. 트랜잭션을 시작할 때 변경안에 담긴 원본과 현재 파일을 비교해
달라졌으면 중단합니다. recover는 현재 파일이 해당 실행의 이전·이후 해시와 모두 다르면
충돌로 중단합니다.
